...
public class GUI  extends SimulationData implements ActionListener  {
	...
	public void UpdateGUI(){
	    if(CenterItem instanceof Graph){
			 DisplayGraph();
            }else if(CenterItem instanceof JScrollPane){
             DisplayActivityLog();
            }else if(CenterItem instanceof JTextArea){
             DisplayNetworkStatistics();
            }
	   }
	...
	public void UpdateTextfields(){
	    ...
	    if(sim.data.size()>0)
		 textfields.get(3).setText("Range: 0-"+String.valueOf(Collections.max(sim.data)));
	   }
